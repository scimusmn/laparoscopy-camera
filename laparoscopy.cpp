...
int main(int argc, char** argv) {
  // important constants
  // check for difference every 10 seconds
  milliseconds diff_check_interval(10000);
  // if no activity for 1 minute, turn off camera
  milliseconds inactive_timeout(30000);
  // move on-screen text every 10 seconds
  milliseconds text_move_interval(10000);
  ...
  // images less than 5% different are deemed the same
  ...
  Mat wait_text =
      make_wait_text(text_fname, alpha_fname, bg_red, bg_grn, bg_blu);
  display =
      make_waitscreen(screen_w, screen_h, wait_text, bg_red, bg_grn, bg_blu);
  ...
  while (true) {
    if (showcamera) {
      ...
      if (steady_clock::now() >= timepoint + inactive_timeout) {
        showcamera = false;
        camera >> frame;
        display = make_waitscreen(screen_w, screen_h, wait_text, bg_red, bg_grn,
                                  bg_blu);
...
Mat make_wait_text(string fwait_text, string falpha_mask, int bg_red,
                   int bg_grn, int bg_blu) {
  // load images
...
Mat make_waitscreen(int screen_w, int screen_h, Mat wait_text, int bg_red,
                    int bg_grn, int bg_blu) {
